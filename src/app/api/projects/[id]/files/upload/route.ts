...
export async function POST(request: Request, context: RouteContext<{ id: string }>): Promise<Response> {
  try {
    ...
    const uploadInput = prepareUploadSchema.parse({
      ...
      manufacturer: optionalFormString(form.get("manufacturer")),
      softwareName: optionalFormString(form.get("softwareName")),
      softwareVersion: optionalFormString(form.get("softwareVersion")),
    ...
    const result = await finalizeFileUpload(user, {
      ...
      manufacturer: uploadInput.manufacturer,
      softwareName: uploadInput.softwareName,
      softwareVersion: uploadInput.softwareVersion,
      originalFileName,
